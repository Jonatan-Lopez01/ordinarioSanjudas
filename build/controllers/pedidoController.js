...
class PedidoController {
    ...
    obtener_mayor_pedido_id(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const consulta = "SELECT MAX(pedido_id) AS mayor_pedido_id FROM pedido";
            const resultado = yield database_1.default.query(consulta);
            if (resultado.length > 0) {
                res.json(resultado[0]);
            }
            else {
                res.status(404).json({ mensaje: "No se encontraron registros en la tabla pedido" });
            }
        });
    }
