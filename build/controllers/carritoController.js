...
class CarritoController {
    ...
    mostrar_productos_carrito_numeroMesa(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const respuesta = yield database_1.default.query('SELECT * FROM carrito WHERE numeroMesa= ? ORDER BY carrito_id ASC', [numeroMesa]);
            res.json(respuesta);
        });
    }
        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (respuesta.length > 0) {
                    // La consulta devolvió algún resultado
                    res.json(respuesta[0]); // Devolver la respuesta como JSON
                }
                else {
                    // La consulta no devolvió ningún resultado
                    res.status(404).json({ message: 'No se encontraron resultados para el número de mesa especificado.' });
                }
            }
            catch (error) {
    ...
    buscar_producto_by_Id(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const { idProducto } = req.params;
            const resp = yield database_1.default.query(`SELECT * FROM carrito WHERE carrito_id = ${idProducto}`);
            res.json(resp[0]);
        });
    }
}
