...
class CarritoBarController {
    ...
    eliminar_bebida_bar_by_idCarrito(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const { CarritoId } = req.params;
            const resp = yield database_1.default.query(`DELETE FROM carrito_bar WHERE carrito_id = ${CarritoId}`);
            res.json(resp);
        });
    }
    ...
    buscar_bebida_by_IdCarrito(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const { idCarrito } = req.params;
            const respuesta = yield database_1.default.query('SELECT * FROM carrito_bar WHERE carrito_id = ?', [idCarrito]);
            if (respuesta.length > 0) {
                res.json(respuesta[0]);
                return;
            }
            res.status(404).json({ 'mensaje': 'Bebida no encontrado por id del carrito' });
        });
    }
}
